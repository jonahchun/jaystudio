                ...
                <td>
                    @switch($data['type'])
                        ...
                        @case('select_tags')
                            ...
                            <select name="{{$name}}[{{$index}}][{{$fieldName}}]" data-tags="true" data-select2-tags="true"
                                    @if(!empty($data['required']))
                                        required
                                @endif
                            >
                                ...
                                @foreach($data['source']->getOptions() as $value => $label)
                                    @php
                                        if (isset($entity->$fieldName) && $entity->$fieldName == $value){
                                            $isCustom = false;
                                        }
                                    @endphp
                                    <option value="{{ $value }}" @if(isset($entity->$fieldName) && $entity->$fieldName == $value) selected="selected" @endif>{{ $label }}</option>
                                @endforeach
                                @if($isCustom)
                                    <option selected="selected">{{$entity->$fieldName}}</option>
                                @endif
                            </select>
                            @break
                        @case('text')
...
<tr data-row-number="{index}">
    ...
    <td>
        @switch($data['type'])
            ...
            @break
            @case('select_tags')
                <select class="form-control" name="{{$name}}[{index}][{{$fieldName}}]" data-tags="true" data-select2-tags="true"
                @if(!empty($data['required']))
                    required
                @endif
                >
