...
<table class="table table-bordered">
    ...
    <tr>
        ...
        <td style="white-space:normal;width:50%;">
            @if(($time = $weddingSchedule->first_newlywed_preparation->jls_start_time) && strpos($time, ':') !== false)
                {{ Illuminate\Support\Carbon::createFromFormat('H:i', $time)->format('g:i A') }}
            @endif
        </td>
    </tr>
    <tr>
        ...
        <td style="white-space:normal;width:50%;">
            @if(($time = $weddingSchedule->first_newlywed_preparation->jls_end_time) && strpos($time, ':') !== false)
                {{ Illuminate\Support\Carbon::createFromFormat('H:i', $time)->format('g:i A') }}
            @endif
        </td>
    </tr>
