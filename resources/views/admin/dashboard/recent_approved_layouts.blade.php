    ...
    <div class="col-lg-12">
        <div class="card bg-dark">
            <div class="card-header bg-transparent text-white border-0">
                {{ __('Recent Approved Layouts') }}
                <div class="card-action">
                    <a class="btn btn-light mb-2" href="{{ route('admin.customer.service') . '?data=' . base64_encode(http_build_query(['status' => \App\Services\Model\Source\Status::DRAFT_EDITS_APPROVED])) }}">
                        {{ __('View All') }}
                    </a>
                </div>
                <div class="table-responsive">
                    <table class="table align-items-center table-flush">
                        ...
                        <tbody>
                        @foreach(\App\Services\Model\Service::where('status', \App\Services\Model\Source\Status::DRAFT_EDITS_APPROVED)->orderBy('created_at', 'desc')->limit(5)->get() as $service)
                            <tr>
                                <td>
